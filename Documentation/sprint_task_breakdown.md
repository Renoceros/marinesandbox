...
* **TASK-TD-102 (Bobo):** Implement the skeleton of `marinesandbox/Views/Canvas/ParallaxScrollView.swift` using deterministic column hashing. Block width is scaled dynamically to exactly **1.5x of the container width** using `GeometryReader` dimensions to avoid deprecated `UIScreen.main` screen properties on iOS 26+. The layers are drawn over a solid color backdrop (#3BAFED) in the order: backdrop -> MG (ratio 0.50, top-pinned) -> BG (ratio 0.20, top-pinned) -> FG (ratio 1.00, bottom-pinned). Note and handle asymmetrical state technical debt (`DEBT-001`).
