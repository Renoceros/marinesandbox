...
* **TASK-TD-102 (Bobo):** Implement the skeleton of `marinesandbox/Views/Canvas/ParallaxScrollView.swift` using deterministic column hashing. Block width is scaled dynamically to exactly **1.5x of the container width** using `GeometryReader` dimensions. The layers are drawn over a solid color backdrop (#3BAFED) in the order: backdrop -> MG (ratio 0.50, top-pinned) -> BG (ratio 0.20, top-pinned) -> FG (ratio 1.00, bottom-pinned). Employs unseamed rendering (no transitions/fades) by binding ForEach views to dynamic column coordinates (`id: \.self`) and applying `.transition(.identity)` to disable implicit SwiftUI entry/exit animations. Note and handle asymmetrical state technical debt (`DEBT-001`).
